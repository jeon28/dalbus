...
| 항목 | 정책 |
| :--- | :--- |
| **배정 방식** | 관리자 수동 매칭 (자동 생성 도구 지원) |
| **신규 주문 자동화** | 신규 고객용 Tidal ID(이메일 기반) 및 비밀번호(연락처 기반) 자동 생성 지원 |
| **배정 시 관리 연동** | 배정 성공 시 해당 계정의 Tidal 관리 페이지 즉시 오픈 (새 탭) |
...
| **재구매 (Renewal)** | 사용자는 동일 서비스에 대해 재구매 가능. <br> - **비회원**: Tidal ID, 이름, 전화번호로 기존 내역 확인 <br> - **회원**: 로그인 계정(ID)으로 본인 내역 확인 |
| **서비스 연장** | 이용 중인 서비스가 있는 상태에서 재구매 시, 기존 만료일에서 선택한 기간만큼 **가입 기간 연장** 처리 |
...
(3) 상품 상세 진입
    ...
    ▼
(4) 기간 선택 + 결제
    │ - 비회원인 경우: 휴대폰 번호 입력 및 기존 이용 내역 확인 (Tidal ID 등) → 로그인/회원가입 유도 또는 게스트 결제
    │ - 회원인 경우: 바로 결제
    │ - **재구매 확인**: 기존 이용 중인 서비스 여부 체크 → 연장 요청으로 처리
    │ - PG사(PortOne) 결제창 호출
    ▼
(5) 결제 완료
...
[관리자 로그인]
    ...
    ▼
(A) 주문 대시보드 확인
    │ - 이메일 알림: 주문 접수 시 관리자 이메일로 자동 통지 (Resend API)
    │ - Status 필터: 주문신청/입금확인/배정완료/작업완료 (Multi-Select)
    │ - 전화번호 검색: 연락처 기반 주문 검색
    │ - Excel 내보내기: 회원/비회원 주문 시트 분리 다운로드
    │ - 신규 '결제완료' 건 확인
    ▼
(B) 계정 배정
    ...
    ▼
(C) Tidal 계정 관리
    │ - 마스터 계정 및 슬롯 관리 (6개 고정 슬롯)
    │ - Excel 내보내기: 마스터 정보 + 슬롯 정보 계층 구조
    ▼
(D) 상품 관리
...
| **SCR_ADM_007** | 만료/비활성 계정 관리 | 만료된 배정 이력 조회, 영구 삭제, 복구(옵션) | Admin |
...
| REQ-3.5 | **P0** | 재구매 및 연장 로직 | 기존 고객 여부 확인(회원:ID / 비회원:Tidal ID + 이름 + 전화번호). 이용 중인 서비스 존재 시 기간 연장 처리. 관리자 대시보드에서 '재구매' 구분 표시 | SCR_SHOP_002, ADM_001 | 대기 |
...
| REQ-6.8 | **P0** | 만료/비활성 계정 관리 | 만료된 배정 자동 식별(UI), 비활성(Soft Delete) 처리, 만료 필터, 지난 내역 조회 및 영구 삭제 | SCR_ADM_002, 007 | 완료 |
...
**이슈 3-2: 재구매 및 서비스 연장 처리**

1. **사용자 스토리**
    - **As a** 기존 달버스 이용자
    - **I want to** 사용하던 계정 그대로 이용 기간을 연장하고 싶다
    - **So that** 새 계정을 배정받는 번거로움 없이 끊김 없는 서비스를 이용하고 싶다
2. **수용 기준**
    - [ ] 구매 시 기존 이용 데이터 조회 로직 수행
        - 비회원: 입력한 이름, 전화번호, Tidal ID 세트가 기존 `order_accounts`에 존재하는지 검증
        - 회원: `user_id` 기준 활성/만료 주문 존재 여부 확인
    - [ ] 활성 서비스가 있는 경우: 주문 데이터 생성 시 `is_extension = true` 및 `related_order_id` 매핑
    - [ ] 어드민 대시보드: '신규' 와 '재구매/연장' 주문 가시적 구분 (뱃지 등)
    - [ ] 연장 주문 배정 시 마스터 계정의 슬롯 기간을 자동/수동 업데이트
...
2. **수용 기준**
    ...
    - [ ] **Grid View (격자 보기)**: 모든 할당 내역(ID, Tidal ID, PW, 구매자 등)을 평면 테이블로 조회, 수정, 이동, 삭제 가능 (토글 버튼 제공)
    - [ ] **주문 번호**: 8자리 난수 포맷 (예: 12345678) 사용
...
| 항목 | 정책 |
| :--- | :--- |
| **무통장 입금 관리** | 관리자가 대시보드에서 직접 입금 계좌(은행, 계좌번호, 예금주)를 관리 |
| **주문 관리** | 주문 내역에서 서비스명과 함께 **이용 기간(구독 개월 수)**을 상시 노출 |
| **모바일 헤더** | 모바일 환경에서 헤더 메뉴(햄버거 버튼)를 상시 노출하여 접근성 강화 |
| **공지/FAQ 카테고리** | 관리자가 공지사항 및 FAQ의 카테고리를 동적으로 추가/삭제/수정 가능 |

## 8.3 요구사항 목록 (Requirements List)

| REQ ID | 우선순위 | 요구사항 | 상세 내용 | 관련 화면 | 상태 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| REQ-1.1 | P0 | 회원가입 (이메일) | 이메일, 비밀번호 입력. 이메일 인증. | SCR_AUTH_001 | 완료 |
| REQ-1.2 | P0 | 로그인 (이메일) | 이메일, 비밀번호 입력. | SCR_AUTH_002 | 완료 |
| REQ-1.3 | P0 | 소셜 로그인 | Google OAuth 연동. | SCR_AUTH_002 | 완료 |
| REQ-1.4 | P0 | 비밀번호 재설정 | 이메일로 재설정 링크 발송. | SCR_AUTH_003 | 완료 |
| REQ-1.5 | P0 | 회원 탈퇴 | 본인 인증 후 탈퇴. | SCR_MY_003 | 완료 |
| REQ-2.1 | P0 | 상품 목록 조회 | 서비스별 상품 목록. 인기/신규/HOT 태그. | SCR_HOME_001 | 완료 |
| REQ-2.2 | P0 | 상품 상세 조회 | 서비스 상세 정보, 가격, 혜택, 주의사항. | SCR_PROD_001 | 완료 |
| REQ-3.1 | P0 | 결제 정보 입력 | 서비스, 기간 선택. 결제 수단 선택 (카드, 무통장). | SCR_PAY_001 | 완료 |
| REQ-3.2 | P0 | 무통장 입금 안내 | 입금 계좌 정보 노출. | SCR_PAY_001 | 완료 |
| REQ-3.3 | P0 | 결제 완료 화면 (Redirect) | 단순 알림창 대신 전용 결과 페이지(`/public/checkout/success`)로 리다이렉트. 주문 요약(서비스명, 이용 기간, 금액, 입금자, 계좌) 표시 | SCR_PAY_002 | 완료 |
| REQ-4.1 | P0 | 주문 내역 조회 | 결제 완료된 주문 리스트. 상태(이용중/만료/취소). | SCR_MY_001 | 완료 |
| REQ-4.2 | P0 | 주문 상세 조회 | 주문 정보, 계정 정보(배정 완료 시). | SCR_MY_002 | 완료 |
| REQ-4.3 | P0 | 만료 예정 알림 | 만료 7일 전, 1일 전 SMS 알림. | SMS | 완료 |
| REQ-4.4 | P0 | 만료 처리 | 만료일 도래 시 자동 만료 처리. | Cron | 완료 |
| REQ-5.1 | P0 | 주문 현황 관리 | 상태별 필터, 서비스별 필터, 날짜/사용자 검색. | SCR_ADM_001 | 완료 |
| REQ-5.2 | P0 | 계정 배정/관리 | '결제완료' 건에 ID/PW 배정. 상태 변경. | SCR_ADM_002 | 완료 |
| REQ-5.3 | P0 | 공유 계정 풀 관리 | 계정 등록, 상태 변경, 배정된 주문 확인. | SCR_ADM_004 | 완료 |
| REQ-5.4 | P0 | 상품/가격 관리 | 서비스별 기간권 가격 수정, 활성/비활성. | SCR_ADM_003 | 완료 |
| REQ-5.6 | P0 | 주문 구분 관리 | 신규/연장 주문 구분 표시. 연장 주문 시 이전 계정 정보 유지. | SCR_ADM_001 | 완료 |
| REQ-5.5 | P0 | 회원 목록 조회 | 전체 회원 리스트, 검색, 상세 조회. | SCR_ADM_MEMBERS | 완료 |
| REQ-6.1 | P0 | 공지사항 목록 | 제목, 날짜, 카테고리. 최신순 정렬. | SCR_NOTICE_001 | 완료 |
| REQ-6.2 | P0 | 공지사항 상세 | 본문(Markdown/Rich Text). | SCR_NOTICE_002 | 완료 |
| REQ-6.3 | P0 | 랜딩 페이지 공지 | 최신 공지 3건 미리보기. | SCR_HOME_001 | 완료 |
| REQ-6.4 | P0 | FAQ 목록 | 아코디언 형태 Q&A. 카테고리 필터. | SCR_FAQ_001 | 완료 |
| REQ-6.5 | P0 | Q&A 게시판 | 회원/비회원 글쓰기, 비밀글, 답변 상태. | SCR_QNA_001 | 완료 |
| REQ-6.6 | P1 | 공지사항/FAQ 관리 | 공지 및 FAQ CRUD. **동적 카테고리 관리 기능** (카테고리 추가/삭제), 노출 순서 관리 | SCR_ADM_005 | 완료 |
| REQ-7.1 | P0 | 채널톡 위젯 | 전체 페이지 우하단 플로팅 버튼. | 전체 | 완료 |
| REQ-7.2 | P0 | 카카오톡 채널 | 고객센터 링크 (Footer, FAQ). | 전체 | 완료 |
...
| v2.1 | 2026-02-08 | 관리자 기능 강화(회원 목록, 서비스 관리), Q&A 게시판(비회원/비밀글) 요구사항 추가 및 화면 정의 업데이트 |
| v2.3 | 2026-02-16 | 만료 계정 관리 시스템 추가 (만료 필터, 비활성화, 지난 내역 조회, DB 스키마 변경) |
| v2.4 | 2026-02-16 | 관리자 전용 회원 메모 기능 및 Tidal 계정 관리 그리드 뷰 고도화 |
| v2.5 | 2026-02-18 | 공용 페이지 로딩 안정화(API 프록시 도입) 및 로그아웃 UX 개선 |
| v2.6 | 2026-02-20 | Tidal 계정 목록 정렬/시각화 고도화 및 프로덕션 빌드 결함 해결 |
    - **As a** 서비스 이용자 (회원/비회원)
    - **I want to** 궁금한 점을 문의하고 관리자의 답변을 받고 싶다
    - **So that** 서비스 이용 중 발생하는 문제를 해결하고 싶다
2. **수용 기준**
    - [ ] **글쓰기**: 
        - 회원: 로그인 연동
        - 비회원: 이름, 비밀번호(수정/삭제용) 필수 입력
    - [ ] **비밀글**: 작성 시 '비밀글' 체크 가능. 작성자와 관리자만 열람 가능
    - [ ] **목록 조회**: 답변 상태(미답변/답변완료) 표시. '비밀글 제외' 필터 제공
    - [ ] **관리자 답변**: 관리자만 답변 등록 가능. 답변 등록 시 상태 '답변완료'로 자동 변경
3. **관련 화면**: SCR_QNA_001, SCR_QNA_002, SCR_ADM_QNA

### [Epic 5] 플랫폼 어드민 운영 관리 (업데이트)

**이슈 5-4: 회원 관리**

1. **사용자 스토리**
    - **As a** 관리자
    - **I want to** 전체 가입 회원 목록을 조회하고 싶다
    - **So that** 회원 현황을 파악하고 연락처 정보를 확인할 수 있다
2. **수용 기준**
    - [ ] 회원 목록 조회: 가입일, 이름, 이메일, 전화번호, 권한 표시
3. **관련 화면**: SCR_ADM_MEMBERS

**이슈 5-3: 상품 및 가격 관리 (상세화)**

1. **수용 기준 업데이트**
    - [ ] 상품(Product) CRUD: 서비스명, 슬러그, 기본 가격, 설명, 이미지 수정
    - [ ] 요금제(Plan) CRUD: 각 상품 하위의 기간별 요금제 추가/수정/삭제
...
| 화면 ID | 화면명 | 주요 구성 요소 | 접근 권한 |
| :--- | :--- | :--- | :--- |
| **SCR_QNA_001** | Q&A 목록 | 문의 리스트, 검색, 비밀글 제외 필터, 글쓰기 버튼 | 전체 |
| **SCR_QNA_002** | Q&A 상세/작성 | 문의 본문, 답변 내용(있으면), 작성/수정 폼(비회원 암호 확인) | 전체 |

### 6.2 관리자 화면 (추가 및 수정)

| 화면 ID | 화면명 | 주요 구성 요소 | 접근 권한 |
| :--- | :--- | :--- | :--- |
| **SCR_ADM_MEMBERS** | 회원 관리 | 전체 회원 리스트 테이블 | Admin |
| **SCR_ADM_SERVICES** | 서비스 관리 | 상품 목록, 상품 추가/수정, 요금제 관리 | Admin |
| **SCR_ADM_QNA** | Q&A 관리 | 전체 문의 조회(비밀글 포함), 답변 작성 폼 | Admin |
