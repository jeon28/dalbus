...
## [v1.6.1] 요금제 중복 등록 오류 해결 (2026.03.07)

### 🛒 요금제 관리 (Plan Management)
- **비활성 요금제 중복 허용**: 동일 상품 내에서 비활성화(Inactive) 상태인 요금제는 개월 수가 중복되어도 등록 및 수정이 가능하도록 개선.
- **활성 요금제 무결성 유지**: '활성화(is_active: true)' 상태인 요금제에 대해서만 개월 수 중복을 방지하는 부분 유니크 인덱스 적용.
- **에러 메시지 최적화**: 활성 요금제 중복 시 "이미 동일한 개월 수의 활성 요금제가 존재합니다."라는 직관적인 한글 메시지 출력.

### 🛠️ 시스템 및 데이터베이스 (System & Database)
- **DB 제약 조건 고도화**: `product_plans` 테이블의 기존 엄격한 UNIQUE 제약 조건을 삭제하고 `idx_product_plans_active_unique` 부분 인덱스 도입.
- **API 안정성**: 요금제 관련 API(`POST`, `PUT`)의 예외 처리 로직을 강화하여 SQL 에러 코드(`23505`)를 정확히 핸들링.

---

## [v1.6.0] 요금제 관리 고도화 및 시스템 안정화 (2026.03.02)

### 🛒 요금제 관리 (Plan Management)
- **삭제 프로세스 단순화**: 불필요한 확인 팝업을 제거하고 즉시 삭제를 시도하도록 개선.
- **예외 기반 유저 안내**: 평소에는 UI를 깔끔하게 유지하고, 주문 내역이 있어 삭제가 불가능한 경우에만 직관적인 안내 문구와 '닫기' 버튼 노출.
- **시각적 피드백**: 삭제 진행 중 아이콘에 애니메이션(Pulse) 스타일을 적용하여 처리 상태 명시.
- **UI 견고화**: 모든 액션 버튼에 `type="button"`을 명시하여 폼 제출이나 페이지 새로고침 간섭 방지.

### 📱 모바일 최적화 (Mobile Optimization)
- **서비스 목록 카드화**: 기존의 테이블 뷰를 모바일 환경에 최적화된 카드 레이아웃으로 교체하여 가독성 향상.
- **상세 페이지 레이아웃 개선**: 싱글 컬럼 그리드 적용 및 터치 조작이 용이하도록 버튼/인풋 여백 조정.
- **반응형 유틸리티 도입**: `mobileOnly`, `desktopOnly` CSS 클래스를 추가하여 기기별 UI 대응력 강화.

### 🛠️ 시스템 안정성 및 빌드 (System Stability & Build)
- **프로덕션 빌드 성공**: `npm run build` 과정에서 발견된 ESLint 미사용 변수(`check-email/route.ts`) 제거 및 빌드 무결성 확인.
- **오류 로깅 정돈**: 삭제 실패(500 에러) 시 브라우저 콘솔에 나타나던 불필요한 에러 객체 출력을 간소화하여 로그 가독성 향상.
- **처리 중 상태(In-Progress) 도입**: 요금제 등록 시 버튼 비활성화 및 처리 중 상태 표시로 중복 클릭 방지.
...
- **마이페이지 개선**: 이메일 우선 배치 및 읽기 전용 설정, 생년월일 수정 기능 활성화.
