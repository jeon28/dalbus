...
export const sendAdminOrderNotification = async (
    ...
    const { orderId, productName, planName, amount, buyerName, buyerPhone, depositorName, orderType, extendTidalId } = order;
    const isExtension = orderType === 'EXT';
    const orderLabel = isExtension ? '연장 주문' : '신규 주문';
    const subject = `[Dalbus] ${orderLabel} 알림 - ${buyerName}님`;
    const html = `
        <h1>${orderLabel}이 접수되었습니다!</h1>
        <p><strong>주문 번호:</strong> ${orderId}</p>
        <hr />
        <h3>주문 상세</h3>
        <ul>
          ${isExtension && extendTidalId ? `<li><strong>연장 Tidal ID:</strong> ${extendTidalId}</li>` : ''}
...
export const sendUserOrderNotification = async (
    ...
    const { orderId, productName, planName, amount, buyerName, depositorName, orderType, extendTidalId } = order;
    const isExtension = orderType === 'EXT';
    const orderLabel = isExtension ? '연장 주문' : '주문';
    ...
    const subject = dynamic?.subject || `[Dalbus] ${orderLabel} 접수 안내 - ${buyerName}님`;
    const html = dynamic?.content || `
        <div style="font-family: sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
          <h2 style="color: #2563eb; border-bottom: 2px solid #2563eb; padding-bottom: 10px;">${orderLabel}이 정상적으로 접수되었습니다.</h2>
          <p>안녕하세요, <strong>${buyerName}</strong>님! Dalbus를 이용해 주셔서 감사합니다.</p>
          <p>주문하신 서비스의 입금이 확인되면 계정 배정이 시작됩니다.</p>

          <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0;">
            ${isExtension && extendTidalId ? `<p style="margin: 5px 0;"><strong>연장 Tidal ID:</strong> ${extendTidalId}</p>` : ''}
