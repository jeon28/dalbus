...
${typeof window !== 'undefined' ? window.location.origin : process.env.NEXT_PUBLIC_SITE_URL}/public`, []);
    ...
    const handlePermanentDelete = async (assignmentId: string) => {
        ...
        try {
            const res = await apiFetch(`/api/admin/assignments/${assignmentId}?hardDelete=true&product=HifiTidal`, { method: 'DELETE' });
                                        ...
                                        return (
                                            <tr key={assignment.id} className={`border-b hover:bg-gray-50 ${!isActive ? 'bg-red-50 text-red-600' : isExpired ? 'bg-red-50/30' : ''} ${selectedAssignmentIds.has(assignment.id) ? 'bg-blue-50/50' : ''}`}>
                                                ...
                                                <td className="p-2 text-center border-r bg-gray-50/5" style={{ width: columnWidths['slot_col'] }}>
                                                    <span className={`font-bold text-xs ${!isActive ? 'text-red-700' : isEmpty ? 'text-green-700' : 'text-blue-600'}`}>
                                                        {acc.login_id}-{sIdx + 1}
                                                    </span>
                                                </td>
